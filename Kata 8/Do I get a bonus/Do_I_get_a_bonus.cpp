...
//
// If bonus is true, the salary should be multiplied by 10. If bonus is false, the fatcat did not make enough moneyand 
...
std::string bonus_time(int salary, bool bonus) {
    std::string result;
    if (bonus) result += "$" + std::to_string(salary * 10);
    else result += "$" + std::to_string(salary);
    return result;
}