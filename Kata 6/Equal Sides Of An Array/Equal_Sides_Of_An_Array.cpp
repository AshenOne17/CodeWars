...
*
 ...
 Your job is to take that arrayand find an index N where the sum of the integers to the left of N is equal to the sum of the integers to the right of N.
...
{
	int index = 0, size = numbers.size();

	int leftSum = 0, rightSum = 0;

	while (index != size)
	{
		leftSum = std::accumulate(numbers.begin(), numbers.begin() + index, 0);
		rightSum = std::accumulate(numbers.begin() + index + 1, numbers.end(), 0);
