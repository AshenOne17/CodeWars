...
//
// Sometimes there are problems : lack of colors, technical malfunctionand a "bad" control string is produced 
...
public:
    static std::string printerError(const std::string& s) {
        int errorCount = 0;
        for (const char ch : s) {
            // If a letter is out of [a, m] range, increment number of errors
            if (!(ch >= 'a' && ch <= 'm')) errorCount++;
        }
