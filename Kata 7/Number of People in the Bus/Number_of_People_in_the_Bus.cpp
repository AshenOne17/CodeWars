// There is a bus moving in the city, and it takesand drop some people in each bus stop.
//
...
unsigned int number(const std::vector<std::pair<int, int>>& busStops) {
    int currentPeople = busStops[0].first;

    // skipping first iteration due to previous assignment and for loop specifics
    bool firstStop = true;
    ...
    for (const auto& stop : busStops) {
        ...
        // people coming into the bus
        currentPeople += stop.first;

        // people leaving the bus
        currentPeople -= stop.second;
    }

    // return the final number of people in the bus
